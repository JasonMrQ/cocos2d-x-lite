...
static bool js_extension_AssetsManagerEx_confirmUpdate(se::State& s)
{
    cocos2d::extension::AssetsManagerEx* cobj = (cocos2d::extension::AssetsManagerEx*)s.nativeThisObject();
    SE_PRECONDITION2(cobj, false, "js_extension_AssetsManagerEx_confirmUpdate : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    if (argc == 0) {
        cobj->confirmUpdate();
...
SE_BIND_FUNC(js_extension_AssetsManagerEx_confirmUpdate)
...
static bool js_extension_AssetsManagerEx_getNeedDownloadSize(se::State& s)
{
    cocos2d::extension::AssetsManagerEx* cobj = (cocos2d::extension::AssetsManagerEx*)s.nativeThisObject();
    SE_PRECONDITION2(cobj, false, "js_extension_AssetsManagerEx_getNeedDownloadSize : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        double result = cobj->getNeedDownloadSize();
        ok &= double_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_extension_AssetsManagerEx_getNeedDownloadSize : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_extension_AssetsManagerEx_getNeedDownloadSize)
...
bool js_register_extension_AssetsManagerEx(se::Object* obj)
{
    ...
    cls->defineFunction("confirmUpdate", _SE(js_extension_AssetsManagerEx_confirmUpdate));
    ...
    cls->defineFunction("getNeedDownloadSize", _SE(js_extension_AssetsManagerEx_getNeedDownloadSize));
